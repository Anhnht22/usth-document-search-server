class BaseCollection {
    ...
    setOnlyActiveRecords(isActive) {
        this.andWhere("t.Active", "=", isActive);
    }
    ...
    sqlCount(is_limit) {
        ...
        let limit = (is_limit == true) ? ` ,${this.limit} AS limits, ${this.page} as pages ` : ` `;
        const sql =
            this.arrayCondition.length === 0
                ? `SELECT count(1) AS total ${limit} FROM ${this.table} t ${this.stringJoin}`
                : `SELECT count(1) AS total ${limit} FROM ${this.table} t ${this.stringJoin} ${condition}`;
        return sql;
    }
    ...
    finallizeTotalCount() {
        let limit = ` ,${this.limit || 0} AS limits, ${this.page || 0} as pages `;
        const condition = this.genCondition();
        // const sql = (this.arrayCondition.length === 0)
        //     ? `SELECT count(*) ${limit} AS total FROM ${this.table} t ${this.stringJoin} ${this.stringGroupBy}`
        //     : `SELECT count(*) ${limit} AS total FROM ${this.table} t ${this.stringJoin} ${condition} ${this.stringGroupBy}`
        //     ;
        ...
        sql = this.stringGroupBy.trim().length > 0
          ? `WITH data AS (SELECT 1 ${sql}) SELECT COUNT(*) AS total ${limit} FROM data`
          : `SELECT count(*) AS total ${limit} ${sql}`
          ;
    ...
    finallizeRoleAccess(is_limit = true) {
        //console.log("Array Condition");
        //console.log(this.arrayCondition);
